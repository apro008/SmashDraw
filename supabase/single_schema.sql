-- SmashDraw single Supabase schema
-- Paste this into the Supabase SQL editor.
-- It is safe for fresh setup and additive updates: it creates missing tables,
-- adds missing columns, refreshes policies/functions/triggers, and avoids dropping data.
...
alter table public.profiles add column if not exists ranking_points int not null default 0;
alter table public.profiles add column if not exists tournaments_played int not null default 0;
...
alter table public.tournaments add column if not exists contact_email text;
...
alter table public.tournaments add column if not exists max_courts int;
...
alter table public.tournament_categories add column if not exists entry_fee int not null default 0;
alter table public.tournament_categories add column if not exists max_players int not null default 32;
alter table public.tournament_categories add column if not exists current_players int not null default 0;
alter table public.tournament_categories add column if not exists skill_level text not null default 'open';
alter table public.tournament_categories add column if not exists prize text;

alter table public.tournament_categories enable row level security;

drop policy if exists "Anyone can view categories" on public.tournament_categories;
create policy "Anyone can view categories" on public.tournament_categories
  for select using (true);

drop policy if exists "Organizers can manage categories" on public.tournament_categories;
...
drop policy if exists "Organizers can view tournament registrations" on public.registrations;
...
alter table public.matches add column if not exists player2_name text;
...
alter table public.matches add column if not exists court_number int;
...
-- RPC: organizer/admin registration status update
...
-- Trigger: keep category counts in sync with approved registrations
